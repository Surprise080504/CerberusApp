...
export async function getMarketPrice({ networkID, provider }: IBaseAsyncThunk) {
  let resp;
  try {
    resp = await axios.get(`https://api.coingecko.com/api/v3/simple/price?ids=cerburus&vs_currencies=usd`);
    return resp.data['cerburus'].usd;
  } catch (e) {
...
export async function getEthPrice() {
  let resp;
  try {
    resp = await axios.get(`https://api.coingecko.com/api/v3/simple/price?ids=ethereum&vs_currencies=usd`);
 ...
 */
export async function getTokenPrice(tokenId = "cerberus") {
  let resp;
  try {
